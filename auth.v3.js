...
function mostrarPantallaLogin() {
    console.log('馃搷 Executant mostrarPantallaLogin...');
    const loginScreen = document.getElementById('login-screen');
    const appScreen = document.getElementById('app-screen');
    
    console.log('馃搷 loginScreen exists?', !!loginScreen);
    console.log('馃搷 appScreen exists?', !!appScreen);
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
    ...
    if (event === 'SIGNED_IN' && session) {
        ...
        // Si ja estem dins l'app i és un refresc de sessió, ignorar
        if (currentUser && currentUser.id === session.user.id) {
            console.log('?? Refresc de sessió ignorat');
            return;
        }
    ...
    try {
       ...
       // Carregar dades necessàries
		...
		if (treballador && role === 'visor') {
			console.log('馃懁 Usuari 茅s treballador visor:', treballador.nom);
			ocultarMenuAdmin();
			carregarVistaTreballadorSimple();
		} else {
			console.log('馃捈 Usuari 茅s admin/editor:', role);
			carregarVistaDashboard();  // O el nombre correcto de la funci髇
}
...
function ocultarMenuAdmin() {
    ...
    // Afegir bot贸 Inici si no existeix
