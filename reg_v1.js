...
// ============================================================
// KC DINÀMIC PER FASE DE COLLITA (CULTIU + VARIETAT)
// ============================================================

const MAPA_CULTIU_FRUITA = {
    'albercoc': 'Albercoc',
    'pressec_juny': 'Préssec Pla',
    'pressec_agost': 'Préssec Pla'
};

// Calendari fenològic per varietat (finestra "normal" de collita)
const CALENDARI_ALBERCOC = {
    'Lido':      { inici: '05-29', fi: '06-06' },
    'Aprireve':  { inici: '05-30', fi: '06-07' },
    'Aprisweet': { inici: '06-18', fi: '06-25' }
};

const CALENDARI_PRESSEC_PLA = {
    'FlatBella':  { inici: '06-24', fi: '07-02' },
    'FlatBeauty': { inici: '07-03', fi: '07-16' },
    'FlatChief':  { inici: '07-08', fi: '07-17' },
    'FlatBoom':   { inici: '07-10', fi: '07-25' }, // corregit (no tan primerenca)
    'FlatReine':  { inici: '07-11', fi: '07-24' },
    'FlatStar':   { inici: '08-05', fi: '08-28' },
    'Guayox 35':  { inici: '08-10', fi: '08-25' }, // corregit (més marge)
    'Nectarina':  { inici: '08-25', fi: '09-05' }  // més marge per més producció
};

function parseMMDDToDate(mmdd) {
...
/**
 * Donat un conjunt de varietats i un calendari, calcula una finestra global
 * (min inici, max fi) per al cultiu, per si no hi ha dades reals de collita.
 */
function obtenirFinestraTeorica(varietats, calendari) {
    let iniciMin = null;
    let fiMax = null;

    for (const v of varietats) {
        const nomVarietat = v.varietat || v.nom || v.varietat_nom;
        const cfg = calendari[nomVarietat];
        if (!cfg) continue;

        const dInici = parseMMDDToDate(cfg.inici);
        const dFi = parseMMDDToDate(cfg.fi);

        if (!iniciMin || dInici < iniciMin) iniciMin = dInici;
        if (!fiMax || dFi > fiMax) fiMax = dFi;
    }

    return { inici: iniciMin, fi: fiMax };
}

/**
 * Retorna la fase de collita per a un cultiu i finca:
 * - precollita_lluny
 * - precollita_propera (≤10 dies abans)
 * - collita
 * - postcollita
 *
 * Usa PRIORITÀRIAMENT dades reals de collita (collita_entrada).
 * Si no n'hi ha, usa calendari fenològic per varietat.
 */
async function getFaseCollita(cultiu, fincaId) {
    const nomFruita = MAPA_CULTIU_FRUITA[cultiu];
    if (!nomFruita) return 'precollita_lluny';

    // 1) Obtenir ID de la fruita (Albercoc, Préssec Pla…)
    const { data: fruita, error: errFruita } = await supabaseClient
        .from('fruites')
        .select('id')
        .eq('nom', nomFruita)
        .single();

    if (errFruita || !fruita) return 'precollita_lluny';

    // 2) Obtenir varietats d’aquesta fruita
    const { data: varietats, error: errVar } = await supabaseClient
        .from('fruita_varietats')
        .select('id, varietat')
        .eq('fruita_id', fruita.id);

    if (errVar || !varietats || varietats.length === 0) return 'precollita_lluny';

    const varietatIds = varietats.map(v => v.id);

    // 3) Buscar entrades de collita d’aquestes varietats i d’aquesta finca (dades reals)
    const { data: collita, error: errCol } = await supabaseClient
        .from('collita_entrada')
        .select('data')
        .in('fruita_varietat_id', varietatIds)
        .eq('finca_id', fincaId)
        .order('data');
    ...
    if (!errCol && collita && collita.length > 0) {
        // PRIORITAT: dades reals de collita
        const dates = collita.map(r => parseDataLocal(r.data));
        inici = dates[0];
        fi = dates[dates.length - 1];
    } else {
        // Sense dades reals → usar calendari fenològic
        const calendari =
            nomFruita === 'Albercoc' ? CALENDARI_ALBERCOC :
            nomFruita === 'Préssec Pla' ? CALENDARI_PRESSEC_PLA :
            null;

        if (!calendari) return 'precollita_lluny';

        const finestra = obtenirFinestraTeorica(varietats, calendari);
        if (!finestra.inici || !finestra.fi) return 'precollita_lluny';

        inici = finestra.inici;
        fi = finestra.fi;
    }

    if (avui < inici) {
        const dies = Math.round((inici - avui) / 86400000);
        return dies <= 10 ? 'precollita_propera' : 'precollita_lluny';
    }

    if (avui >= inici && avui <= fi) {
        return 'collita';
    }
...
/**
 * Ajusta el Kc segons la fase de collita.
 * - precollita_lluny / collita → Kc original
 * - precollita_propera → Kc * 0.80
 * - postcollita → Kc * 0.45
 */
function ajustarKcPerFase(kc, fase) {
    if (fase === 'precollita_propera') return kc * 0.80;
    if (fase === 'postcollita') return kc * 0.45;
    return kc;
}
...
async function carregarDadesReg(finques, dataInici, dataFi) {
    ...
    try {
        ...
        for (let finca of finques) {
            ...
            let kc = await getRegKc(finca.cultiu, mes);

			// KC DINÀMIC SEGONS FASE DE COLLITA (cultiu + finca)
		try {
			// Assumim que a reg_configuracio tens un camp finca_id (UUID de la finca)
			const fase = await getFaseCollita(finca.cultiu, finca.finca_id);
			kc = ajustarKcPerFase(kc, fase);
			} catch (e) {
			console.warn('No s’ha pogut calcular fase de collita per KC dinàmic:', e);
}

			// Ajust dinàmic segons fase de collita
			const fase = await getFaseCollita(finca.cultiu, finca.nom_finca);

			if (fase === 'precollita') kc *= 0.80;      // -20%
			if (fase === 'postcollita') kc *= 0.45;     // -55%
